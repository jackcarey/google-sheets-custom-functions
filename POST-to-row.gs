...
const doPost = (event = {}) => {
  ...
  const contents = postData.contents ? JSON.parse(postData.contents) : {};
  if(!contents){
    contents = {};
  }
  contents.doPostTimestamp = new Date();
  ...
  try {
    ...
    keys.forEach(key => {
      ...
      if (idx != -1) {
        sheet.getRange(lastRowNum, idx + 1).setValue(contents[key]);
      }
    })
